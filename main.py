...
def loading_sequence(chat_id): for msg in [ "🛠️ Yuklanmoqda... / Загрузка...", "⏳ Maʼlumotlar tekshirilmoqda... / Проверка данных...", "🤖 Bot ishga tushirilmoqda... / Инициализация бота...", "🚀 Tayyorlanmoqda... / Подготовка..." ]: bot.send_message(chat_id, msg) time.sleep(2) send_keyboard(chat_id, translations["uz"]["start"], [["🇺🇿 O'zbek", "🇷🇺 Русский"]])
...
if "salary" not in data:
    try:
        ...
        send_keyboard(chat_id, translations[lang]["other_loans"], [["✅ Ha", "❌ Yo'q"]] if lang == "uz" else [["✅ Да", "❌ Нет"]])
    except:
...
def calculate_diff_and_send(chat_id): data = user_data[chat_id] lang = data["lang"] salary = data["salary"] months = data["months"] rate = data["rate"] / 100 other = data.get("other_loans_amount", 0)
